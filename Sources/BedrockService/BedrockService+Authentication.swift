...
/// - `webIdentity`: Use a web identity token (JWT) to assume an IAM role. This is useful for applications running ion iOS, tvOS or macOS where you cannot use the AWS CLI. Typically, the application authenticates the user with an external Identity provider (such as Apple Sign In) and receives a JWT token. The application then uses this token to assume an IAM role and get AWS credentials. Some additional configuration is required on your AWS account to allow this. See https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_providers_oidc.html for more information. If you use Sign In With Apple, read https://docs.aws.amazon.com/sdk-for-swift/latest/developer-guide/apple-integration.html for more information.
///      `webidentity` is often used by application presenting a user interface. It allows you to pass an optional closure that will be called when the credentials are retrieved. This is useful for updating the UI or notifying the user. The closure will be called on the main thread.
/// - `static`: Use static AWS credentials. We strongly recommend to not use this option in production. This is useful for testing and debugging purposes.
